...
(function () {
    ...
    RemoteImage.prototype = {
        ...
        initContainer: function () {
		    ...
		    this.setImage(img);
//zhu:act:20240701184423|多图上传(insertimage)适配图片base64
		    //zhu:如果图片已存在，把图片原始width/height存入隐藏域中
		    $G('img_sour_width').value = img.width;
		    $G('img_sour_height').value = img.height;
		    $G('lock').disabled = false;
            }
        },
        initEvents: function () {
            var _this = this,
...
//zhu:act:20240701184423|多图上传(insertimage)适配图片base64
		//zhu:这里注册一下“还原”链接的事项
		domUtils.on($G("reset_sour_wh"), 'click', function () {
			//把宽高输入框的值变成图片原本的宽高
			$G('width').value = $G('img_sour_width').value;
			$G('height').value = $G('img_sour_height').value;
		});
...
//zhu:act:20240701184423|多图上传(insertimage)适配图片base64
                ...
                // $G("width").value = img.width || '';
		//     $G("height").value = img.height || '';
		//zhu:这里改为img的style中原本有width/height才回显到表单中
		$G("width").value = '';
                $G("height").value = '';
		if (img.style.width) {
			//注：这里不能用 img.style.width（因为里面会带有“px”）
			$G("width").value = img.width;
		}
		if (img.style.height) {
			$G("height").value = img.height;
