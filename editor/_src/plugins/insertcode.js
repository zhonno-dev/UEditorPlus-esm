...
UE.plugins["insertcode"] = function () {
	...
	//zhu:here:实现：[代码语言]框（<pre>）中键入Tab时保留Tab（不转成空格）
	//zhu:在[代码语言]框（<pre>）内按Tab键时会触发（回调）：
	//zhu:定位搜索： var pre = domUtils.findParentByTagName(rng.startContainer, 'pre',
    me.addListener("tabkeydown", function (cmd, evt) {
        var rng = me.selection.getRange();
        var pre = domUtils.findParentByTagName(rng.startContainer, "pre", true);
	//zhu:让[代码语言]框（<pre>）中按Tab键时所键入的依然是Tab（而不是变成4个空格）
	// var tab_str = '    '; //这是原本按Tab键时所输入的4个空格
	var tab_str = '	'; //这是Tab（即按Tab时原本输入的字符）
        if (pre) {
            me.fireEvent("saveScene");
            if (evt.shiftKey) {
            } else {
                if (!rng.collapsed) {
                    ...
                    while (start) {
                        if (pre.firstChild === start && !domUtils.isBr(start)) {
                            pre.insertBefore(me.document.createTextNode(tab_str), start);

                            break;
                        }
                        if (domUtils.isBr(start)) {
                            pre.insertBefore(
                                me.document.createTextNode(tab_str),
                                start.nextSibling
                            );
                    ...
                    var tmpNode = me.document.createTextNode(tab_str);
                    rng
                        .insertNode(tmpNode)
